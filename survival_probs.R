...
plot.survival.prob.versus.gens = function(WF.det.list,WF.sto.list,dmipos,chrlen,recomb) {
    ...
    v = chrlen*dmipos[1]
    w = chrlen*dmipos[2]
    A.trajecs  = WF.sto.list$A.matrix
    survival.time.matrix = matrix(ncol=nrow(A.trajecs),nrow=ncol(A.trajecs))
    deterministic.vector = c(1:nrow(A.trajecs))
    pb = txtProgressBar(1,ncol(survival.time.matrix)-1,1,style=3)
    for (i in 1:(ncol(survival.time.matrix)-1)) {
        gens.prob = i
    ...
    close(pb)
    par(mar=c(5,5,4,2))
    matplot(t(survival.time.matrix[,c(1:10)]),type='l',col="cornflowerblue",lty=1,
            ylab="Survival Probability",xlab="Generations",lwd=2,ylim=c(0,.5),cex.lab=1.7)
    #lines(deterministic.vector[c(1:10)],lwd=2,lty=2)
    return(survival.time.matrix)
}

plot.survival.prob.versus.gens.again = function(WF.det.list,WF.sto.list,dmipos,chrlen,recomb) {
    r =  recomb
    v = chrlen*dmipos[1]
    w = chrlen*dmipos[2]
    A.trajecs  = WF.sto.list$A.matrix
    ...
    close(pb)
    par(new=T)
    matplot(t(survival.time.matrix[,c(1:10)]),type='l',col="orange",lty=1,
            ylab="",xlab="",lwd=2,ylim=c(0,.5),xaxt='n',yaxt='n')
    #legend('topright', c("Neutral","Selection"),col= c("cornflowerblue","orange"),lwd=2,cex=1.5)
    #lines(deterministic.vector[c(1:10)],lwd=2,lty=2)
    return(survival.time.matrix)
}
