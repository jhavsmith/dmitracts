...
run_comparisons(popsize,chrlen,migrate,nmarkers,tstart,dmipos,seldmi,selanc,domdmi,reps,demefile,episfile,outfile.a,outfile.b,chrnum)                     
                                                 
#-----plot neutral versus selection distributions--------             
             
plot_distributions(outfile.a,outfile.b,reps,bins)   

#--------------------------------------------------------    


plot_distributions = function(outfile.a,outfile.b,reps,bins) {
    recover()
    ...
    # now the tracts
    tracts.df = make_tracts_df(tracts.sel,tracts.neu,bins)
    # plotting
    ...
    a = ggplot(aes(y = Counts, x = as.character(Length), fill = Simulation), data = tracts.a.a) + geom_boxplot()
    b = ggplot(aes(y = Counts, x = as.character(Length), fill = Simulation), data = tracts.b.a) + geom_boxplot()
    c = ggplot(aes(y = Counts, x = as.character(Length), fill = Simulation), data = tracts.a.b) + geom_boxplot()
    d = ggplot(aes(y = Counts, x = as.character(Length), fill = Simulation), data = tracts.b.b) + geom_boxplot()
    #extract legend
    mylegend = g_legend(a)
    p3 <- grid.arrange(arrangeGrob(a + theme(legend.position="none"),
                                   ...
                                   nrow=2), mylegend, nrow=3)
    
    p3
  #  p = ggplot(aes(y = Ancestry, x = Locus, fill = Simulation), data = anc.df) + geom_boxplot() + theme(legend.position="top")
  #  q = ggplot(aes(y = Junctions, x = Locus, fill = Simulation), data = jun.df) + geom_boxplot() + guides(fill=FALSE)
  #  grid.arrange(p,q,nrow=2)         
}
 
make_tracts_df = function(tracts.sel,tracts.neu,bins) {
    tracts.df = data.frame(Replicate = integer(),Length = integer(),Counts = integer(), 
                           Ancestry = character(),Simulation = character(),Locus = character())
    for (i in 1:reps) {
        ...
        sel.hist.a.a = hist(sel.rep.a[which(sel.rep.a[,2]==0),],plot=F,breaks=bins)
        sel.hist.a.b = hist(sel.rep.a[which(sel.rep.a[,2]==1),],plot=F,breaks=bins)
        sel.hist.b.a = hist(sel.rep.b[which(sel.rep.b[,2]==0),],plot=F,breaks=bins)
        sel.hist.b.b = hist(sel.rep.b[which(sel.rep.b[,2]==1),],plot=F,breaks=bins)        
        neu.hist.a.a = hist(neu.rep.a[which(neu.rep.a[,2]==0),],plot=F,breaks=bins)
        neu.hist.a.b = hist(neu.rep.a[which(neu.rep.a[,2]==1),],plot=F,breaks=bins)
        neu.hist.b.a = hist(neu.rep.b[which(neu.rep.b[,2]==0),],plot=F,breaks=bins)
        neu.hist.b.b = hist(neu.rep.b[which(neu.rep.b[,2]==1),],plot=F,breaks=bins)   
        sel.a.a = data.frame(Replicate = rep(i,length(sel.hist.a.a$counts)), Length = sel.hist.a.a$breaks[-1],
                             Counts = sel.hist.a.a$counts, Ancestry = rep("A",length(sel.hist.a.a$counts)),
        ...
        sel.a.b = data.frame(Replicate = rep(i,length(sel.hist.a.b$counts)), Length = sel.hist.a.b$breaks[-1],
                             Counts = sel.hist.a.b$counts, Ancestry = rep("B",length(sel.hist.a.b$counts)),
        ...
        sel.b.a = data.frame(Replicate = rep(i,length(sel.hist.b.a$counts)), Length = sel.hist.b.a$breaks[-1],
                             Counts = sel.hist.b.a$counts, Ancestry = rep("A",length(sel.hist.b.a$counts)),
                             Simulation = rep("Selection",length(sel.hist.b.a$counts)), 
                             Locus = rep("Locus 2",length(sel.hist.a.a$counts)))
        sel.b.b = data.frame(Replicate = rep(i,length(sel.hist.b.b$counts)), Length = sel.hist.b.b$breaks[-1],
                             Counts = sel.hist.b.b$counts, Ancestry = rep("B",length(sel.hist.b.b$counts)),
        ...
        neu.a.a = data.frame(Replicate = rep(i,length(neu.hist.a.a$counts)), Length = neu.hist.a.a$breaks[-1],
                             Counts = neu.hist.a.a$counts, Ancestry = rep("A",length(neu.hist.a.a$counts)),
        ...
        neu.a.b = data.frame(Replicate = rep(i,length(neu.hist.a.b$counts)), Length = neu.hist.a.b$breaks[-1],
                             Counts = neu.hist.a.b$counts, Ancestry = rep("B",length(neu.hist.a.b$counts)),
        ...
        neu.b.a = data.frame(Replicate = rep(i,length(neu.hist.b.a$counts)), Length = neu.hist.b.a$breaks[-1],
                             Counts = neu.hist.b.a$counts, Ancestry = rep("A",length(neu.hist.b.a$counts)),
                             Simulation = rep("Neutral",length(neu.hist.b.a$counts)), 
                             Locus = rep("Locus 2",length(neu.hist.a.a$counts)))
        neu.b.b = data.frame(Replicate = rep(i,length(neu.hist.b.b$counts)), Length = neu.hist.b.b$breaks[-1],
                             Counts = neu.hist.b.b$counts, Ancestry = rep("B",length(neu.hist.b.b$counts)),
...
junction_count = function(haprep,chrnum,windowsize,dmipos) {
    ...
    if (chrnum==2) {
        ...
        junctions = list("junctions.a"=junctions.a,"junctions.b"=junctions.b,"mean.a"=mean.b,"mean.b"=mean.b)     
    }
    return(junctions)
} 
...
run_comparisons = function(popsize,chrlen,migrate,nmarkers,tstart,dmipos,seldmi,selanc,domdmi,reps,demefile,episfile,outfile.a,outfile.b,chrnum) {
    # The neutral simulation:     
    run_dfuse(popsize,chrlen,migrate,nmarkers,tstart,dmipos,seldmi=0,selanc=0,domdmi=0,reps,demefile,episfile,outfile.a,chrnum)
    compute_summary_stats(outfile.a, dmipos, chrnum, windowsize)            
    # The selection simulation:         
    run_dfuse(popsize,chrlen,migrate,nmarkers,tstart,dmipos,seldmi,selanc,domdmi,reps,demefile,episfile,outfile.b,chrnum)
